...
def addUser(username, email, roles):
	...
	headers = {'Content-Type': 'application/xml', 'Cookie': token, 'Accept': ACCEPT_TYPE, 'Accept-Language': getSysLocale().replace("_", "-").lower()}
	try:
		response, content = http.request(url_useradd, 'POST', body = body, headers=headers)
...
def removeUser(username):
	...
	url_userdel = url + 'ws/users/' + username
	
	headers = {'Content-Type': 'application/xml', 'Cookie': token, 'Accept': ACCEPT_TYPE, 'Accept-Language': getSysLocale().replace("_", "-").lower()}
	try:
		response, content = http.request(url_userdel, 'DELETE', headers=headers)
